...
- `commit create` prints a dry run first. In an interactive terminal it can commit and push after a yes/no prompt, then asks whether to run `warroom pr create` next after a successful commit. From War Room root it can infer the target repo from the single active mapped development branch written by `issue next`; pass `--repo <id>` if multiple child repos could match. Non-interactive use still requires `--confirm`. Pass `--no-push` only when a local-only commit is intentional. Validation commands must pass before a commit proceeds.
